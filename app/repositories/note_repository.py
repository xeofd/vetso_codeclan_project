...
def save(note):
    ...
    sql = "INSERT INTO note (date, note_text, note_id, vet_id) VALUES (%s, %s, %s, %s) RETURNING id"
    values = [note.date, note.note_text, note.note.id, note.vet.id]
...
def select_all():
    ...
    for row in results:
        ...
        new_note = Note(row['first_name'], row['last_name'], pet, vet, row['id'])
...
def select(note_id):
    ...
    if len(result) > 0:
        ...
        new_note = Note(result[0]['name'], result[0]['dob'], pet, vet)
